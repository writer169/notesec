// pages/notes/index.js (ВАРИАНТ 1 - с useSession)
...
// import { getServerSideProps } from '../../lib/auth'; // УДАЛЯЕМ

export default function Notes() { //  НЕ принимаем session в props
  const { data: session, status } = useSession(); // Используем useSession
  ...
  // const [isMounted, setIsMounted] = useState(false); // isMounted больше не нужен

  // Fetch notes (only if there's a session and an encryption key)
    const fetchNotes = useCallback(async () => {
    if (!session || !encryptionKey) { // Исправлено: проверяем session, а не activeSession
      setLoading(false);
      return;
    }
        ...
        data.map(async (note) => {
          if (note.content) {
            try {
              ...
              // Можно вернуть заметку без content, пометить её как повреждённую, и т.д.
              return { ...note, content: null, decryptionError: true };
            }
          }
          return note; // Если content нет, возвращаем заметку как есть
        })
      );
  ...
  }, [session, encryptionKey]); // Исправлено: session, а не activeSession

    // Load notes when encryption key changes
  useEffect(() => {
    if (encryptionKey) { //Исправлено убрал isMounted
      fetchNotes();
    }
  ...
  // Setup master password
    ...
    if (!session) { // Исправлено: проверяем session
      alert('Сессия не найдена. Пожалуйста, войдите снова.');
      return;
    }

    try {
      const salt = session.user.email; // Исправлено: используем email из сессии
        ...
        try {
            ...
            if (note.content) {
                try {
                    ...
                    return { ...note, content: null }; // Возвращаем заметку без расшифрованного контента
                }
            }
 ...
 const saveNote = async (noteData) => {
    try {
      ...
      if (method === 'PUT') {
        ...
        setNotes((prevNotes) => [
          { _id: updatedNote._id, title: updatedNote.title, updatedAt: updatedNote.updatedAt },
          ...prevNotes,
        ]);
          // Устанавливаем новую заметку как выбранную (без расшифрованного содержимого)
          setSelectedNote({
          _id: updatedNote._id,
          title: updatedNote.title,
...
const exportNotes = async () => {
  ...
  if (status === "unauthenticated") {
      router.replace("/"); // Делаем редирект
      return null; // Предотвращаем рендеринг
  }
...
// export { getServerSideProps }  УДАЛЯЕМ
