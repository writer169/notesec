...
export default function Notes() {
  ...
  // Редирект, если не аутентифицирован (только после монтирования)
  useEffect(() => {
  ...
  // Загрузка заметок (только если есть сессия и ключ)
  ...
  // Загрузка заметок при изменении ключа
  useEffect(() => {
    if (isMounted && encryptionKey) { // Also check isMounted
      fetchNotes();
    }
  ...
  // Остальные функции (createNewNote, selectNote, editNote, fetchFullNote, saveNote, deleteNote, importNotes, exportNotes) остаются без изменений
  // ... (Вставьте сюда остальные ваши функции: createNewNote, selectNote, ... exportNotes) ...
    const createNewNote = () => {
